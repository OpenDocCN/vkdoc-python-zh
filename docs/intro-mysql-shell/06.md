...
`JSON_PRETTY()`函数格式化 JSON 文档以便于查看。您可以用它来生成一个输出发送给用户，或者让 JSON 在 shell 中看起来更好一些。清单 6-21 显示了一个不带功能和带功能的例子。注意当使用`JSON_PRETTY()`时，阅读变得容易多了。
...
然而，有一件事这个例子没有涉及到。如果 JSON 数据元素是一个字符串，您必须使用`JSON_UNQUOTE()`函数从字符串中删除引号。假设我们想要为颜色数据元素添加一个生成的列。如果我们添加带有`ALTER TABLE`语句的列和索引而不删除引号，我们将得到一些不寻常的结果，如清单 6-22 所示。
