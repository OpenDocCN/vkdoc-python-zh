...
要导入文档，可以使用 shell 连接到服务器，然后使用`util`内置类和`import_json()`方法指定要导入的文件的路径和一个包含模式和集合的选项字典。清单 7-20 演示了将一个带有 JSON 文档的文件导入到`garage_v2`模式中的`vendors`集合中。注意，对于导入大量(JSON)数据来说，import 是一个非常方便的实用工具。
