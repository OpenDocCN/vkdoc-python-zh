...
`__future__`模块支持许多这样的特性，Python 的每个版本都添加了新的选项。在本书的其余部分，我将在所描述的特性足够新以至于需要在 Python 的旧版本(回到 Python 2.5)中进行`__future__`导入时提到它们，而不是试图在这里列出它们。关于这些特性变化的完整细节可以在 Python 文档的“新特性”页面上找到。 <sup>1</sup>
