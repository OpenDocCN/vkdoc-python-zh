...
运行这个程序并访问`http://localhost:8080/1`会得到一个类似图 3-1 的网页。
...
Klein 的电镀提供了一个独特的特性:您可以通过指定`json`查询参数来请求将 slots 字典作为序列化的 JSON 返回。在图 3-3 中，我们可以看到当提供这个参数时，我们的“慢增量”页面是什么样子。
